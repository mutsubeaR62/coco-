...
def get_store_code() -> str:
    """現在ログイン中ユーザーの店舗コードを返す（未ログイン時は 'default'）"""
    return (st.session_state.get("user") or {}).get("store_code") or "default"

def store_path(filename: str) -> str:
    """店舗コードをプレフィックスとしたJSONキーを返す"""
    return f"{get_store_code()}/{filename}"
...
def _migrate_to_store_scope():
    """既存データを 'default' 店舗スコープに移行（初回のみ実行）"""
    ...
    for fname in _STORE_FILES:
        old = _load_json_cached(fname)
        if old is not None and _load_json_cached(f"default/{fname}") is None:
            try:
                save_json(f"default/{fname}", old)
            except Exception:
                pass

def update_store_code(old_code: str, new_code: str):
    """店舗コードを変更し、全ユーザーとデータを新コードに移行する"""
    ...
    for fname in _STORE_FILES:
        ...
        if old_data is not None and _load_json_cached(f"{new_code}/{fname}") is None:
            try:
                save_json(f"{new_code}/{fname}", old_data)
            except Exception:
                pass
    _load_json_cached.clear()
...
def init_default_users():
    ...
    if not data.get("users"):
        ...
        # 既存ユーザーへの店舗コード付与 & データ移行
        _migrate_to_store_scope()
...
def get_user_by_username(username):
    """全店舗を横断してユーザーを検索する（ログイン・Cookie復元用）"""
...
def add_user(username, password, name, role, store_code=None,
             employee_type=None, hourly_wage=None,
...
def get_progress(username):
    data = load_json(store_path("progress.json"), {})
...
def save_progress(username, progress):
    ...
    save_json(store_path("progress.json"), data)
...
def get_today_birthdays(exclude_username=None):
    """今日が誕生日のユーザーリストを返す（同じ店舗のみ）。"""
    from datetime import datetime
    today_mmdd = datetime.now().strftime("%m-%d")
    result = []
    for u in get_all_users():   # 店舗フィルタ済み
        bd = u.get("birthday", "")
...
def get_store_settings():
    return load_json(store_path("store_settings.json"), {
        "store_name": "CoCo壱番屋",
        "store_branch": "",
    })
