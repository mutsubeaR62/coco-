    ...
    for cl in checklists:
        ...
        with st.expander(header, expanded=not all_done):
            ...
            for i, item in enumerate(items):
                ...
                with col_check:
                    new_val = st.checkbox("", value=checks[i],
                                          key=f"cl_{cl_name}_{i}",
                                          label_visibility="collapsed")
                with col_label:
                    style = "text-decoration: line-through; color: #999;" if checks[i] else ""
                    st.markdown(f"<div style='{style} padding-top:4px;'>{item['task']}</div>",
                ...
                if show_why:
                    st.markdown(f'<div class="why-box">🔑 {item["why"]}</div>',
                                unsafe_allow_html=True)
    ...
    if _is_manager:
        ...
        with st.expander("📎 ファイル添付（管理者・代行）"):
            ...
            for cl_item in cl_data.get("checklists", []):
                ...
                upload_attachment_ui("checklist", cl_item["id"])
                st.divider()
...
with tab_stepup:
    ...
    def count_stage(stage, prog):
        ...
        checked = sum(
            1 for s in stage.get("sections", [])
            for it in s["items"]
            if prog.get(f"{stage['id']}::{s['name']}::{it}")
        )
    ...
    if not stages:
        st.info("ステップアップ表のデータがありません。")
    else:
        # 対象メンバー選択
        if _is_manager:
            all_users = get_all_users()
            user_opts = {
                f"{ROLE_LABELS.get(u.get('role',''), '')} {u['name']}": u["username"]
                for u in all_users
            }
            sel_name = st.selectbox("対象メンバー", list(user_opts.keys()),
                                     key="stepup_target")
            target_un = user_opts[sel_name]
        else:
            target_un = username
        ...
        all_checked = sum(
            sum(1 for s in stage.get("sections", [])
                for it in s["items"]
                if su_prog.get(f"{stage['id']}::{s['name']}::{it}"))
            for stage in stages
        )
        if all_total > 0:
...
<div style='background:white;border-radius:12px;padding:14px 20px;
  ...
  <div style='background:#f0f0f0;border-radius:99px;height:12px;'>
    <div style='background:linear-gradient(90deg,#e85d04,#f48c06);
                width:{pct}%;height:12px;border-radius:99px;'></div>
  </div>
</div>
""", unsafe_allow_html=True)
        ...
        updated = False
        for stage in stages:
            checked, total = count_stage(stage, su_prog)
            stage_pct = int(checked / total * 100) if total > 0 else 0
            color = stage.get("color", "#e85d04")
            completed = (checked == total and total > 0)

            with st.expander(
                f"{'✅ ' if completed else ''}{stage['title']}　{stage.get('target','')}　{checked}/{total}",
                expanded=not completed
            ):
                st.markdown(f"""
<div style='background:#f8f9fa;border-radius:8px;padding:8px 12px;margin-bottom:10px;'>
  <div style='display:flex;justify-content:space-between;font-size:0.85rem;margin-bottom:4px;'>
    <span style='color:{color};font-weight:700;'>{stage['title']}</span>
    <span style='color:#666;'>{checked}/{total} ({stage_pct}%)</span>
  </div>
...
""", unsafe_allow_html=True)
                ...
                for sec in stage.get("sections", []):
                    ...
                    for item in sec["items"]:
                        ...
                        cur = su_prog.get(key, False)
                        if _is_manager:
                            new_val = st.checkbox(
                                item, value=cur,
                                key=f"su_{target_un}_{key}"
                            )
                            if new_val != cur:
                                su_prog[key] = new_val
                                updated = True
                        else:
                            st.markdown(f"{'✅' if cur else '⬜'} {item}")

        if updated and _is_manager:
            su_prog["_updated_at"] = datetime.now().strftime("%Y-%m-%d %H:%M")
            su_prog["_updated_by"] = user["name"]
            save_stepup_progress(target_un, su_prog)
            st.success("保存しました！")
            st.rerun()
